...
begin
  ...
  insert into public.message_attachment_private(
    message_id, conversation_id, company_id, storage_bucket, storage_path
  ) values (
    v_message_id, p_conversation_id, v_conv.company_id, 'widget-attachments', p_storage_path
  );
...
-- Repair any existing browser-visible locators that still have valid messages.
insert into public.message_attachment_private(message_id,conversation_id,company_id,storage_bucket,storage_path)
select
  m.id,
  m.conversation_id,
  c.company_id,
  m.metadata->>'storage_bucket',
  m.metadata->>'storage_path'
from public.messages m
join public.conversations c on c.id=m.conversation_id
where m.content_type in ('image','video','file')
  and c.company_id is not null
  and m.metadata ? 'storage_bucket'
  and m.metadata ? 'storage_path'
  and m.metadata->>'storage_bucket'='widget-attachments'
  and coalesce(m.metadata->>'storage_path','')<>''
on conflict (message_id) do nothing;

update public.messages
set metadata = metadata - 'storage_bucket' - 'storage_path'
where content_type in ('image','video','file')
  and (metadata ? 'storage_bucket' or metadata ? 'storage_path');
...
declare
  ...
  bad integer;
begin
  select count(*) into leaked from public.messages
   where content_type in ('image','video','file')
     and (metadata ? 'storage_bucket' or metadata ? 'storage_path');
  if leaked <> 0 then raise exception 'browser-visible attachment locators remain: %', leaked; end if;

  select count(*) into bad
    from public.message_attachment_private p
    join public.conversations c on c.id=p.conversation_id
   where p.company_id is distinct from c.company_id;
  if bad <> 0 then raise exception 'attachment private company lineage mismatch: %',bad; end if;
end $$;
