-- Task 2 correction: permit direct postgres maintenance only when no JWT identity exists.
-- This keeps policy/helper calls functional while preventing an authenticated caller from
-- probing another user's company membership or role through exposed SECURITY DEFINER RPCs.
