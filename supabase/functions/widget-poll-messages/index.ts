...
import { hasActiveThinkingClaim, isHumanControlState } from "../_shared/widget-runtime-state.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: thinking, error: thinkingError } = await supabase.from("messages").select("id, created_at, metadata")
      .eq("conversation_id", conversation_id).eq("content", "__THINKING__").eq("is_recalled", false);
