 ...
 *
 * Security posture
 *   - exact Bearer parsing; a malformed Authorization header is rejected, never
 *     forwarded to the auth client
 *   - hard body-size limit read before JSON parsing
 *   - strict request shape: unknown fields are rejected, not ignored
 *   - tenant authorization: the caller must be an active member of the company
 *     that owns the conversation, with a role permitted for the action
 *   - fixed public error contract: callers receive a code from a closed set and
 *     never receive env names, RPC internals or database messages
 *
 * Reliability posture
 *   - provider retry and timeout live in the governed model router
 *   - grounding failures abort before any attempt row is created
 *   - completion ambiguity is resolved by reading back the evaluation row
 *   - a failed persist always leaves the attempt terminal, never running
...
async function readBackEvaluation(
  ...
  const { data } = await admin
    ...
    .select(
      "id, conversation_id, overall_score, severity, review_status, training_eligible, " +
        "has_verified_human_response, input_snapshot_hash, bundle_hash, evaluation_contract_version, created_at",
    )
...
async function handleEvaluate(
  ...
  // The snapshot is what makes the evaluation replayable. Bodies are redacted
  // before persistence so the snapshot never becomes a second copy of raw PII.
  // The snapshot stores the actual redacted canonical input that was sent to
  // every evaluator, so Replay Studio can reconstruct the exact model input.
  // The bundle_hash can be recomputed from canonical_input to verify integrity.
  const snapshot = {
...
async function handleReview(
  ...
  // review_evaluation derives the actor from auth.uid(), so it must run on the
  // caller's connection, not on the service-role connection.
  ...
  try {
    for (const key of REQUIRED_ENV) {
      ...
      if (!v || v.trim().length === 0) {
        // The missing variable is logged server side and never named to the caller.
