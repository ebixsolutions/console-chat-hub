...
Deno.serve(async(req)=>{
  ...
  try{
    const supabaseUrl=Deno.env.get("SUPABASE_URL"),anonKey=Deno.env.get("SUPABASE_ANON_KEY"),serviceRoleKey=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");if(!supabaseUrl||!anonKey||!serviceRoleKey)return jsonResponse({error:"server_config_missing"},500,req);
    ...
    const admin=createClient(supabaseUrl,serviceRoleKey);const roles=await loadGlobalRoles(admin,user.id);if(!roles)return jsonResponse({error:"role_lookup_failed"},500,req);if(!roles.some(r=>ALLOWED_ROLES.has(r)))return jsonResponse({error:"forbidden"},403,req);
    ...
    const endpoint=resolveKBEndpoint();if(!endpoint)return jsonResponse({error:"kb_config_missing"},500,req);const result=await fetchKBRag({query,top_k:topK},scope,endpoint);
