...
const CORRECTION = /(我講錯|我说错|我說錯|我要更正|我想更正|更正一下|其實係|其实是|改返|改成|actually[,\s]+i meant|\bi meant\b|correction\s*[:：]|不是.+(?:而)?是|唔係.+係|not .+ but .+)/i;
const SIMPLIFY = /(?:簡單|简单)(?:一點|一点|啲|些|點|点)?.*(?:解釋|解释|講|讲|說|说|介紹|介绍)|explain(?: it| that)? (?:more )?simply|make it simpler|\bsimpler\b|\bshorter\b/i;
...
const SUMMARY = /(?:總結|总结|概括|歸納|归纳).*(?:剛才|刚才|以上|之前|我們|我们|內容|内容|三點|三点)?|summari[sz]e(?: that| it| this| the above| what we discussed| our conversation)?/i;
const MEMORY = /(一開始|一开始|第一個問題|第一个问题|最初).*(?:問|問題|问题)|剛才.*(?:建議|建议|叫我|要我)|刚才.*(?:建议|叫我|要我)|之前.*(?:建議|建议|提供)|what\s+(?:did\s+i\s+ask|was\s+(?:my\s+)?first)|what\s+did\s+you\s+(?:recommend|suggest)|what\s+information\s+have\s+i\s+already\s+given|what\s+have\s+i\s+already\s+given|what.*still\s+missing/i;
