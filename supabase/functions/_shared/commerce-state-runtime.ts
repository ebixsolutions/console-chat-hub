...
interface NegatedTransactionScan { positive: string; negated_order: boolean; negated_payment: boolean; negated_booking: boolean; }

function scanNegatedTransaction(text: string): NegatedTransactionScan {
  let positive = text;
  const removed: string[] = [];
  for (const pattern of NEGATED_TX_PATTERNS) {
    positive = positive.replace(pattern, (match) => { removed.push(match); return " "; });
  }
...
function explicitOrderConfirmation(text: string): boolean { return /(?:已付款|已付|付咗|paid\b|正式落單|正式下单|確認落單|确认下单|confirm(?:ed)? (?:the )?order|已預約|已预约|booked)/i.test(text); }
function explicitPaymentConfirmation(text: string): boolean { return /(?:已付款|已付|付咗|已支付|paid\b)/i.test(text); }
function explicitBookingConfirmation(text: string): boolean { return /(?:已預約|已预约|已約|已约|已安排|booked|scheduled)/i.test(text); }
function quotationOnlySignal(text: string): boolean { return /(?:報價|报价|quotation|quote|未落單|未下单|未正式|唔係落單|不是下单|先問價|先问价)/i.test(text); }
...
export function extractCommerceCalculationTerms(texts: string[], state: ConversationCommerceState, options?: { include_historical_state?: boolean }): { terms: CommerceCalculationTerm[]; currency: string | null } {
  const amounts: number[] = [];
  for (const raw of texts) {
    const text = clean(raw);
    if (!text) continue;
  ...
  for (const amount of amounts) {
    ...
    if (count < 2) { seen.set(amount, count + 1); unique.push(amount); }
  }
  ...
  const stateMultiplier = activeEntities.length === 1 ? Math.max(1, activeEntities[0].quantity) : activeEntities.reduce((sum, entity) => sum + Math.max(0, entity.quantity), 0) || 1;
  const multiplier = currentTurnMultiplier ?? stateMultiplier;
  const currency = state.quotes.find((q) => q.currency)?.currency ?? "HKD";
  return { terms: unique.map((amount, index) => ({ label: `customer_term_${index + 1}`, value: amount, multiplier })), currency };
}
...
function isHistoricalMoneyContext(text: string): boolean {
  const t = clean(text);
  if (!t) return false;
  return /(?:之前|以前|以往|舊|旧|歷史|历史|previous|historical|earlier|old)/i.test(t)
    && /(?:報價|报价|價|价|price|quote|quotation|收費|收费|fee|HK\$|HKD|\$|元|蚊|dollars?)/i.test(t);
}

export function selectCommerceCalculationTexts(currentText: string, historyTexts: string[]): { texts: string[]; include_historical_state: boolean } {
  ...
  if (explicitlyHistorical) return { texts: [current, ...historyTexts.map((text) => clean(text)).filter(Boolean)], include_historical_state: true };
  if (parseMoneyTerms(current).length > 0 || detectCurrentPriceValidityQuestion(current)) return { texts: [current], include_historical_state: false };
...
interface LoadedCommerceState { state: ConversationCommerceState; revision: number; }

export async function loadCommerceState(db: CommerceStateDbClient, conversation_id: string): Promise<LoadedCommerceState> {
  ...
  return { state: isConversationCommerceState(state) ? state : createEmptyConversationCommerceState(), revision: Number.isFinite(revision) && revision > 0 ? revision : 0 };
}

function deriveA3RuntimeEvents(input: CommerceRuntimeInput, hints: CommerceTurnEntityHint[], previous: ConversationCommerceState): CommerceStateEvent[] {
  ...
  if (correction && quantity !== null && mentioned.length === 0) {
    const active = previous.entities.filter((entity) => entity.status !== "cancelled" && entity.status !== "deferred");
    if (active.length === 1) events.push({ type: "SET_ENTITY_QUANTITY", entity_id: active[0].entity_id, quantity, provenance });
  }
  if (additive && quantity !== null && mentioned.length === 0) {
    const active = previous.entities.filter((entity) => entity.status !== "cancelled" && entity.status !== "deferred");
    if (active.length === 1) {
      events.push({ type: "SET_ENTITY_QUANTITY", entity_id: active[0].entity_id, quantity: active[0].quantity + quantity, provenance });
      events.push({ type: "SET_ENTITY_STATUS", entity_id: active[0].entity_id, status: "tentative", provenance });
    }
  }
  for (const hint of mentioned) {
    if (cancelled) { events.push({ type: "SET_ENTITY_STATUS", entity_id: hint.entity_id, status: "cancelled", provenance }); continue; }
    if (deferred) { events.push({ type: "SET_ENTITY_STATUS", entity_id: hint.entity_id, status: "deferred", provenance }); continue; }
    ...
    if (quantity !== null || explicitCreation) events.push({ type: "SET_ENTITY_STATUS", entity_id: hint.entity_id, status: "tentative", provenance });
  }
...
export function enforceQuotationNotOrderEvents(text: string, state: ConversationCommerceState): CommerceStateEvent[] {
  ...
  const events: CommerceStateEvent[] = [];
  if (scan.negated_booking && !positiveBooking && state.delivery.confirmed) events.push({ type: "SET_DELIVERY", patch: { confirmed: false }, provenance: { source_type: "derived" } });
  ...
  if (!promoted && !paidDrift) {
    if (quotationOnlySignal(text) && state.conversion.quotation_status === "none") events.push({ type: "SET_CONVERSION", patch: { funnel_stage: "quotation", quotation_status: "draft" } });
    return events;
  }
  const patch: Partial<ConversationCommerceState["conversion"]> = { funnel_stage: "quotation", quotation_status: state.conversion.quotation_status === "none" ? "draft" : state.conversion.quotation_status };
...
export function detectExplicitEntityCreationSignal(text: string): boolean {
  const t = clean(text); if (!t) return false; if (parseCount(t) !== null) return true;
...
export function filterGhostUnscopedHints(text: string, state: ConversationCommerceState, hints: CommerceTurnEntityHint[]): CommerceTurnEntityHint[] {
  const explicitCreation = detectExplicitEntityCreationSignal(text);
  const additive = detectAdditiveEntityCreationSignal(text);
  ...
  return hints.filter((hint) => {
    ...
    const hasConcreteSameCategory = state.entities.some((entity) => entity.category === hint.category && !entity.entity_id.endsWith(":unscoped") && entity.status !== "cancelled" && entity.status !== "deferred");
    if (rooms.length === 0 && hasConcreteSameCategory) return false;
    return true;
  });
}

export function reduceTurn(previous: ConversationCommerceState, input: CommerceRuntimeInput, rawHints: CommerceTurnEntityHint[]): ConversationCommerceState {
  ...
  const derivedRaw = calculationTurn || semanticAuthoritative ? [] : deriveCommerceEventsFromCustomerTurn({ text: input.text, source_message_id: input.source_message_id, occurred_at: input.occurred_at ?? null, entity_hints: hints, current_language: input.language });
...
export async function persistCommerceTurn(db: CommerceStateDbClient, input: CommerceRuntimeInput, hints: CommerceTurnEntityHint[]): Promise<{ state: ConversationCommerceState; revision: number; result: string }> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    const { data, error } = await db.rpc(COMMERCE_STATE_RPC, { p_conversation_id: input.conversation_id, p_company_id: input.company_id, p_expected_revision: expected, p_source_message_id: input.source_message_id, p_state: next });
    ...
    if (parsed.result === "revision_conflict" && attempt === 0) { const reloaded = await loadCommerceState(db, input.conversation_id); expected = reloaded.revision; next = reduceTurn(reloaded.state, input, hints); continue; }
...
function statusLabel(status: string, language: CommerceLanguage): string {
  const table: Record<string, Record<CommerceLanguage, string>> = {
    cancelled: { "zh-TW": "已取消", "zh-CN": "已取消", en: "cancelled" }, deferred: { "zh-TW": "暫緩", "zh-CN": "暂缓", en: "deferred" }, confirmed: { "zh-TW": "已確認", "zh-CN": "已确认", en: "confirmed" }, tentative: { "zh-TW": "初步", "zh-CN": "初步", en: "tentative" }, researching: { "zh-TW": "考慮中", "zh-CN": "考虑中", en: "under consideration" },
  };
  return table[status]?.[language] ?? status;
}

export function buildTransactionSummary(state: ConversationCommerceState, language: CommerceLanguage): string {
  const lines: string[] = [];
  ...
  const t = {
    head: { "zh-TW": "我幫你整理咗現時已確認嘅資料：", "zh-CN": "我帮你整理了目前已确认的资料：", en: "Here is what is on record so far:" }, items: { "zh-TW": "項目", "zh-CN": "项目", en: "Items" }, none: { "zh-TW": "暫時未有", "zh-CN": "暂时没有", en: "none yet" }, removed: { "zh-TW": "已取消／暫緩", "zh-CN": "已取消／暂缓", en: "Cancelled / deferred" }, delivery: { "zh-TW": "送貨安排", "zh-CN": "送货安排", en: "Delivery" }, pending: { "zh-TW": "待師傅上門確認", "zh-CN": "待师傅上门确认", en: "Pending onsite professional checks" }, quotes: { "zh-TW": "你提供嘅歷史報價（歷史數字，非現價）", "zh-CN": "你提供的历史报价（历史数字，非现价）", en: "Historical prices you provided (historical, not current)" }, status: { "zh-TW": "目前狀態", "zh-CN": "目前状态", en: "Current status" }, tail: { "zh-TW": "最新價格、適用費用同相關條件仍然要確認之後先作準。", "zh-CN": "最新价格、适用费用及相关条件仍需确认后才作准。", en: "Latest pricing, applicable fees and relevant conditions still need to be confirmed." },
  } as const;
  ...
  if (language === "en") { lines.push(orderConfirmed ? "Order: confirmed." : "Order: not yet confirmed."); lines.push(paymentPaid ? "Payment: received." : "Payment: no confirmed payment on record yet."); }
  else if (language === "zh-CN") { lines.push(orderConfirmed ? "订单：已确认。" : "订单：尚未确认。"); lines.push(paymentPaid ? "付款：已确认收到。" : "付款：目前未有已付款记录。"); }
  else { lines.push(orderConfirmed ? "訂單：已確認。" : "訂單：尚未確認。"); lines.push(paymentPaid ? "付款：已確認收到。" : "付款：目前未有已付款記錄。"); }
...
function buildQuantityAnswer(state: ConversationCommerceState, language: CommerceLanguage): string | null {
  const active = state.entities.filter((e) => e.status !== "cancelled" && e.status !== "deferred"); if (!active.length) return null;
  const total = active.reduce((sum, e) => sum + e.quantity, 0); const breakdown = active.map((e) => `${entityLabel(e.entity_id, language)} x${e.quantity}`).join("、");
  if (language === "en") return `You currently have ${total} unit(s) in total: ${breakdown}.`; if (language === "zh-CN") return `你目前合共 ${total} 个单位：${breakdown}。`; return `你而家合共 ${total} 個單位：${breakdown}。`;
}
...
function buildKnownStateAnswer(language: CommerceLanguage, statePath: string, value: unknown, state: ConversationCommerceState): string {
  if (statePath.startsWith("entities.") && statePath.endsWith(".quantity")) { const answer = buildQuantityAnswer(state, language); if (answer) return answer; }
  const rendered = typeof value === "object" ? JSON.stringify(value) : clean(String(value), 300);
  if (language === "en") return `From what you already told me: ${rendered}.`; if (language === "zh-CN") return `按你之前提供的资料：${rendered}。`; return `按你之前提供嘅資料：${rendered}。`;
}

function formatCalculationNumber(value: number): string { return new Intl.NumberFormat("en-US", { maximumFractionDigits: 2 }).format(value); }
function renderCalculationExpression(calculation: { expression: string; result: number }): string {
  const parsed = calculation.expression.split(" + ").map((part) => { const match = part.match(/^[^:]+:([0-9.]+)×([0-9.]+)$/); return match ? { value: Number(match[1]), multiplier: Number(match[2]) } : null; });
  if (parsed.length && parsed.every(Boolean)) { const terms = parsed as Array<{ value: number; multiplier: number }>; const multiplier = terms[0].multiplier; if (terms.every((term) => term.multiplier === multiplier)) { const values = terms.map((term) => formatCalculationNumber(term.value)).join(" + "); return `${formatCalculationNumber(multiplier)} × (${values}) = ${formatCalculationNumber(calculation.result)}`; } }
...
function buildCalculationAnswer(language: CommerceLanguage, calculation: { expression: string; result: number; currency?: string | null }): string {
  const currency = calculation.currency ?? "HKD"; const rendered = renderCalculationExpression(calculation);
  if (language === "en") return `Based only on the figures in this calculation: ${rendered} (${currency}). Latest prices, applicable fees and conditions still need to be confirmed.`; if (language === "zh-CN") return `只按你这次提供的数字计算：${rendered}（${currency}）。最新价格、适用费用及相关条件仍需确认。`; return `只按你今次提供嘅數字計：${rendered}（${currency}）。最新價格、適用費用同相關條件仍然要確認。`;
}

function buildProfessionalConfirmationAnswer(language: CommerceLanguage, state: ConversationCommerceState): string {
  const active = state.entities.filter((e) => e.status !== "cancelled" && e.status !== "deferred"); const known = active.length ? active.map((e) => `${entityLabel(e.entity_id, language)} x${e.quantity}`).join("、") : "";
  if (language === "en") return `${known ? `I still have your details on record: ${known}. ` : ""}For safety and accuracy, our technician needs to inspect the site in person before we can confirm whether the installation is suitable.`; if (language === "zh-CN") return `${known ? `我们已保留您之前提供的资料：${known}。` : ""}为确保安全和准确，需要师傅上门检查窗口尺寸、承托及安装环境后再确认是否适合安装。`; return `${known ? `我哋已保留您之前提供嘅資料：${known}。` : ""}為確保安全同準確，需要師傅上門檢查窗口尺寸、承托同安裝環境後先可以確認是否適合安裝。`;
}

export function buildCurrentPriceValidityAnswer(language: CommerceLanguage): string { if (language === "en") return "Not necessarily. A previous quote is only a reference and does not guarantee the current price. The latest price, applicable fees and relevant conditions need to be confirmed again before they are final."; if (language === "zh-CN") return "未必。你之前看到的报价只可作为参考，并不代表目前仍是同一价格。最新价格、适用费用及相关条件需要重新确认后才作准。"; return "未必。你之前見過嘅報價只可以作參考，唔代表而家仍然係同一個價。最新價格、適用費用同相關條件需要重新確認後先作準。"; }
export function buildPreorderUnpaidAnswer(language: CommerceLanguage, state: ConversationCommerceState): string { const active = state.entities.filter((e) => e.status !== "cancelled" && e.status !== "deferred"); const known = active.length ? active.map((e) => `${entityLabel(e.entity_id, language)} x${e.quantity}`).join("、") : ""; const nextStep = buildCapabilityAwarePreorderNextStep(state, language); if (language === "en") return `${known ? `Got it — you want to reserve ${known}. ` : "Got it — you want to proceed. "}Since payment has not been made yet, this is not a completed or confirmed order. ${nextStep}`; if (language === "zh-CN") return `${known ? `好的，我知道你想预订${known}。` : "好的，我知道你想继续预订。"}由于目前还未付款，所以现在还不算已完成或已确认订单。${nextStep}`; return `${known ? `好，我知道你想預訂${known}。` : "好，我知道你想繼續預訂。"}因為你仲未付款，所以而家未算完成或已確認訂單。${nextStep}`; }

export async function runCommerceStateRuntime(db: CommerceStateDbClient, input: CommerceRuntimeInput): Promise<CommerceRuntimeOutcome | null> {
  const text = clean(input.text); if (!text || !input.conversation_id || !input.company_id || !input.source_message_id) return null;
  const language = input.language;
  if (detectPreorderUnpaidIntent(text)) { const loaded = await loadCommerceState(db, input.conversation_id); const hasActiveEntity = loaded.state.entities.some((entity) => entity.status !== "cancelled" && entity.status !== "deferred"); if (hasActiveEntity) return { authority: "CONVERSATION_STATE", reply: buildPreorderUnpaidAnswer(language, loaded.state), revision: loaded.revision, persist_result: "read_only", reason: "preorder_intent_acknowledged_without_order_or_payment_promotion", route: "commerce_state_answer" }; }
  ...
  const deterministicHints = buildCommerceEntityHints(conversationTexts); const semanticHints = semanticFrameToEntityHints(input.semantic_frame); const hints = mergeCommerceEntityHints(semanticHints, deterministicHints);
  const persisted = await persistCommerceTurn(db, input, hints); const state = persisted.state;
  const summaryIntent = detectTransactionSummaryIntent(text); const wantsCalculation = detectExplicitCalculationRequest(text);
  const calculationContext = wantsCalculation ? selectCommerceCalculationTexts(text, historyTexts) : { texts: [text], include_historical_state: false };
  const calculation = wantsCalculation ? extractCommerceCalculationTerms(calculationContext.texts, state, { include_historical_state: calculationContext.include_historical_state }) : { terms: [] as CommerceCalculationTerm[], currency: null };
  const decision = resolveCommerceAnswerAuthority({ question: text, state, calculation_terms: calculation.terms, calculation_currency: calculation.currency, requires_professional_site_check: requiresProfessionalSiteCheck(text) });
  const base = { revision: persisted.revision, persist_result: persisted.result, reason: decision.reason };
  if (decision.authority === "SAFE_PROFESSIONAL_CONFIRMATION") return { ...base, authority: decision.authority, reply: buildProfessionalConfirmationAnswer(language, state), route: "commerce_state_answer" };
  if (detectPreorderUnpaidIntent(text)) return { ...base, authority: "CONVERSATION_STATE", reason: "preorder_intent_acknowledged_without_order_or_payment_promotion", reply: buildPreorderUnpaidAnswer(language, state), route: "commerce_state_answer" };
  if (decision.authority === "CONVERSATION_STATE" && decision.state_path) return { ...base, authority: decision.authority, state_path: decision.state_path, reply: buildKnownStateAnswer(language, decision.state_path, decision.known_value, state), route: "commerce_state_answer" };
  if (decision.authority === "DETERMINISTIC_CALCULATION" && decision.calculation) return { ...base, authority: decision.authority, calculation: decision.calculation, reply: buildCalculationAnswer(language, decision.calculation), route: "commerce_state_answer" };
  if (detectCurrentPriceValidityQuestion(text)) return { ...base, authority: "CURRENT_KB_REQUIRED", reason: "previous_quote_not_authoritative_for_current_price", reply: buildCurrentPriceValidityAnswer(language), route: "commerce_state_answer" };
  if (summaryIntent && (state.entities.length > 0 || state.quotes.length > 0)) return { ...base, authority: "CONVERSATION_STATE", reply: buildTransactionSummary(state, language), route: "commerce_transaction_summary" };
  return { ...base, authority: decision.authority === "CURRENT_KB_REQUIRED" ? "CURRENT_KB_REQUIRED" : decision.authority, reply: null, route: "commerce_kb_required" };
}
