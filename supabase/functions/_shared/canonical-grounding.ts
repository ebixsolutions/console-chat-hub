...
function modelTokens(text: string): string[] {
  return [
    ...new Set(
      (text.match(/\b[A-Z]{2,}[A-Z0-9]*[- ]?\d{2,}[A-Z0-9-]*\b/g) ?? []).map((
        x,
...
function unique(values: Array<string | null | undefined>): string[] {
  return [
    ...new Set(
      values.map((value) => value?.normalize("NFKC").trim()).filter((
        value,
...
function namedTargetTokens(text: string): string[] {
  ...
  for (
    const match of text.matchAll(/\b(?:smoke\s+test\s+)?(basic|growth|pro)\b/gi)
  ) {
  ...
  // Canonical commerce objects are semantic targets too. A formatting verb in
  // the same turn (for example, "summarize order TEST-C1") must never cause an
  // earlier product answer to be reused for this newly named object.
  for (
    const match of text.matchAll(
...
function assessApplicability(
  ...
  if (
    requestJurisdiction === "mars" && !documentJurisdictions.includes("mars")
  ) {
    return {
      accepted: false,
...
function strongLexicalEvidenceMatch(
  requestText: string,
  document: KBDocumentCandidate,
): boolean {
  ...
  const namedPlan = ["growth", "basic", "pro"].find(
    (plan) =>
      new RegExp(`\\b${plan}\\b`, "i").test(request) &&
      new RegExp(`\\b${plan}\\b`, "i").test(text),
  );
  const requestHasPlanFact =
    /(?:sku|staff|admin|seat|app|push|crm|會員|会员|ai\s*seo|price|billing|monthly|yearly|limit|上限|費用|费用|價錢|价钱|價格|价格)/i
      .test(
        request,
      );
  ...
  if (
    reqModels.length > 0 &&
    reqModels.some((model) => text.includes(model.toLowerCase()))
  ) {
    return /(?:price|spec|warranty|保養|保修|噪音|db|delivery|shipping|送貨|送货)/i
      .test(request);
  }
  return false;
}
...
export function selectCanonicalGrounding(
  ...
  const expectedRegion = options.expectedRegion ?? target.region ??
    detectExplicitJurisdiction(requestText);
  ...
  for (const document of documents ?? []) {
    ...
    if (
      document.llm_context.full_content_evidence.some((e) =>
    ...
    const effectiveMinScore =
      strongLexicalEvidenceMatch(currentTargetRequest, document)
    ...
    const fullContentIds = new Set(
      chunks.filter((c) => c.chunk_type === "full_content").map((c) =>
    ...
    if (
      requirePublished && evidence.length > 0 &&
      chunks.every((c) => c.status !== "published")
    ) {
    ...
    const effectiveCurrentness = declaredCurrentness === "unknown"
    ...
    eligible.push({
      ...
      authorityCandidate: {
        ...
        authority_class: effectiveCurrentness === "current"
          ? "CURRENT_KB"
          : "HISTORICAL",
        ...
        claims: metadata?.claims?.length
          ? metadata.claims
          : extractStructuredClaims(evidence),
      },
    });
  }
  ...
  const authorityDecision = resolveReferenceAuthority({
    ...
    require_explicit_target_match: target.target_changed ||
      target.explicit_entity || target.explicit_topic ||
      Boolean(expectedRegion),
  });
  const winner = authorityDecision.selected_source_id
    ? eligible.find(
      (candidate) =>
        candidate.document.document_id ===
          authorityDecision.selected_source_id &&
