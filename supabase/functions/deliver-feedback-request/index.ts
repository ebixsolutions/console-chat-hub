...
//
// Dev19a patch (2026-07-08):
//   Adds trace logging to legacyGenerateReply() ONLY.
//   Writes upstream_call_log + final_prompt_trace after assistant message insert.
//   Non-blocking: try/catch, never delays AI reply.
//   Schema-aligned to actual types.ts columns (v1.2 corrected).
//   Does NOT modify orchestrationGenerateReply() or any other function.
...
// ── Dev19a: Trace logging helpers (schema-aligned to types.ts v1.2) ───────
function sanitizeUserMessage(msg: string): string {
  if (!msg) return "";
  let s = msg.slice(0, 300);
...
async function writeTraces(
  ...
  params: {
    ...
    lastVisitorMsg: string;
    insertedMessageId: string | null;
  },
): Promise<void> {
  try {
    // upstream_call_log — actual schema columns
    await supabaseAdmin.from("upstream_call_log").insert({
      upstream_service: "anthropic",
      conversation_id: params.conversation_id,
      response_status: params.responseStatus,
      response_latency_ms: Math.max(0, params.responseTimestamp - params.requestTimestamp),
      error_message: params.errorMessage,
      request_payload: {
        model: params.model,
        token_input: params.tokenInput,
        token_output: params.tokenOutput,
        request_timestamp: new Date(params.requestTimestamp).toISOString(),
        response_timestamp: new Date(params.responseTimestamp).toISOString(),
        prompt_source: "minimal_fallback",
      },
    });
  } catch (traceErr) {
    console.warn("[generate-reply] upstream_call_log write failed (non-blocking):", traceErr);
  }

  // final_prompt_trace — only on success path (has message context)
  if (!params.errorMessage) {
    try {
      await supabaseAdmin.from("final_prompt_trace").insert({
        conversation_id: params.conversation_id,
        message_id: params.insertedMessageId,
        model_used: params.model,
        system_prompt_snapshot: "[legacy_inline_cs_prompt_v1]",
        token_input: params.tokenInput,
        token_output: params.tokenOutput,
        latency_ms: Math.max(0, params.responseTimestamp - params.requestTimestamp),
        rag_context: null,
        tool_calls: null,
        user_message: sanitizeUserMessage(params.lastVisitorMsg),
      });
    } catch (traceErr) {
      console.warn("[generate-reply] final_prompt_trace write failed (non-blocking):", traceErr);
    }
  }
}
// ── End Dev19a helpers ─────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────────
// LEGACY PATH — preserved L5a behavior + Task A.1A handoff + Dev19a traces.
// ⚠️ Do NOT add adapter calls / overlay reads / status changes here.
// Dev19a: Added trace logging (upstream_call_log + final_prompt_trace).
//         Trace writes are non-blocking (try/catch). Never delays AI reply.
// ────────────────────────────────────────────────────────────────────────────
async function legacyGenerateReply(conversation_id: string): Promise<Response> {
  ...
  const SYSTEM_PROMPT = `You are a professional and friendly customer service assistant. 
Answer customer questions clearly and concisely. 
If you cannot answer a question confidently, acknowledge it honestly and offer to connect the customer with a human agent.
Keep responses under 150 words.
...
When the customer explicitly requests a human agent, or when you transfer to a human agent, include a short safe handoff status message in the same language and script as the customer. The message must state that the conversation has been recorded and that a human agent will reply in this same chat after taking over. If the customer is using Traditional Chinese, use: "我們已將你的對話記錄，客服接手後會在此對話中回覆你。目前未啟用即時輪候時間顯示。" If the customer is using Simplified Chinese, use: "我们已将你的对话记录，客服接手后会在此对话中回复你。目前未启用实时排队位置和预计等待时间显示。" If the customer is using English, use: "We have recorded your conversation. A human agent will reply in this same chat after taking over. Real-time queue position and estimated wait time are not currently enabled." Do NOT invent estimated wait times, response-time promises, or queue positions.`;
  ...
  // ── Dev19a: Capture request timestamp ────────────────────────────────────
  const requestTimestamp = Date.now();

  // ── Dev19a: Wrap fetch in try/catch for network/timeout exceptions ──────
  ...
  try {
    ...
    // Dev19a C4: Network/timeout/exception — write trace then return error
    const responseTimestamp = Date.now();
    const errorMsg =
      fetchError instanceof DOMException && fetchError.name === "AbortError" ? "timeout" : "api_exception";

    await supabaseAdmin.from("conversations").update({ ai_generating: false }).eq("id", conversation_id);

    // Best-effort trace for exception path
    ...
    console.error("[generate-reply] fetch exception:", errorMsg);
  ...
  // ── Dev19a: Capture response timestamp ──────────────────────────────────
  const responseTimestamp = Date.now();
  ...
  if (!claudeResponse.ok) {
    ...
    // Dev19a: Trace for API error path
    await writeTraces(supabaseAdmin, {
      conversation_id,
      requestTimestamp,
      responseTimestamp,
      ...
      tokenInput: null,
      tokenOutput: null,
      errorMessage: `api_error_${claudeResponse.status}`,
      lastVisitorMsg,
      insertedMessageId: null,
    });
  ...
  if (!aiReplyContent) {
    ...
    // Dev19a: Trace for empty response
    await writeTraces(supabaseAdmin, {
      conversation_id,
      requestTimestamp,
      responseTimestamp,
      responseStatus: claudeResponse.status,
      model: MODEL,
      tokenInput: claudeData?.usage?.input_tokens ?? null,
      tokenOutput: claudeData?.usage?.output_tokens ?? null,
  ...
  // Dev19a: Capture insertedMessageId from message insert
  ...
  const insertedMessageId = insertedMsg?.id ?? null;
  ...
  // Dev19a: Best-effort trace write (after reply delivered, non-blocking)
  await writeTraces(supabaseAdmin, {
    conversation_id,
    requestTimestamp,
    responseTimestamp,
