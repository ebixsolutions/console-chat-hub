...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: conv, error: convErr } = await supabaseAdmin
      .from("conversations").select("id, status, assigned_agent_id")
    ...
    if (!isElevated) {
      const { data: ap, error: apErr } = await supabaseAdmin
        .from("agent_profile").select("id").eq("user_id", user.id).maybeSingle();
      if (apErr || !ap) {
    ...
    if (toolType === "translate") {
      ...
      return jsonRes({ success: true, tool_type: "translate", result: {
    ...
    if (toolType === "grammar") {
      ...
      return jsonRes({ success: true, tool_type: "grammar", result: {
    ...
    if (toolType === "suggest_reply") {
      ...
      const safe = (p.suggestions as Array<Record<string, unknown>>)
        .filter(s => typeof s.content === "string" && String(s.content).trim().length > 0 && VALID_SUG_TONES.has(String(s.tone_label)))
        .slice(0, 3)
        .map(s => ({ content: String(s.content).slice(0, 1000), tone_label: String(s.tone_label) }));
      if (safe.length === 0) {
        return jsonRes({ success: false, error: "suggest_parse_failed" }, 502, req);
      }
      return jsonRes({ success: true, tool_type: "suggest_reply", draft_only: true, result: { suggestions: safe } }, 200, req);
    }

    if (toolType === "check_policy") {
      if (!policyContext || policyContext.length === 0) {
        return jsonRes({ success: true, tool_type: "check_policy", result: {
