 ...
 try{
  ...
  if(toolType==="suggest_reply" && body.context_mode!=="manual" && conv.company_id){
    ...
    if(!recallHistoryError && recallSource){
      ...
      servicePlan=planConversationService({
        question:content,language:/[\u4e00-\u9fff]/.test(content)?"zh-TW":"en",
        recall:{handled:recallRoute.decision.handled,reason:recallRoute.decision.handled?undefined:recallRoute.decision.reason,detail:recallRoute.decision.handled?undefined:recallRoute.decision.detail},
        memory:c3Memory,commerce:recallSnapshot?.state??null,
        recent_messages:(recallHistory??[]).slice().reverse().map((row:any)=>({role:String(row.role),content:String(row.content??"")})),
      });
  ...
  if(contextMode==="conversation"){const history=await loadAssistConversationHistory(supabaseAdmin,conversationId);if(history===null)return jsonRes({success:false,error:`${kbPrefix}_conversation_context_unavailable`},500,req);retrievalQuery=(servicePlan?.kb_query||buildCanonicalAssistRetrievalQuery([content,c3Memory?.current_goal,c3Memory?.current_topic].filter(Boolean).join(" "),history).query).slice(0,500);}
