...
function ReviewConsole({ canRun, canReview }: { canRun: boolean; canReview: boolean }) {
  ...
  const channelOptions = useMemo(() => {
    ...
    rows.forEach((r) => {
      if (r.channel_name) set.add(r.channel_name);
    });
    return [...set].sort();
  }, [rows]);
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (reviewStatus !== "any") {
        if (r.review_status === null) return false;
        if (r.review_status !== reviewStatus) return false;
      }
      ...
      <div className="flex h-full min-h-0 flex-col gap-4 lg:flex-row">
        <div className="flex min-h-0 flex-col gap-3 overflow-y-auto lg:basis-[40%]">
          <div className="flex flex-wrap items-center gap-2">
            <TabPill
              label={t(C.pills.all)}
              count={error ? null : counts.total}
              active={pill === "all"}
              onClick={() => setPill("all")}
            />
            <TabPill
              label={t(C.pills.evaluated)}
              count={error ? null : counts.evaluated}
              active={pill === "evaluated"}
              onClick={() => setPill("evaluated")}
            />
            <TabPill
              label={t(C.pills.notEvaluated)}
              count={error ? null : counts.not_evaluated}
              active={pill === "not_evaluated"}
              onClick={() => setPill("not_evaluated")}
            />
            <TabPill
              label={t(C.pills.needsReview)}
              count={error ? null : counts.needs_review}
            ...
            <span className="text-[11px] text-slate-500">
              {error ? (
                ...
                <>
                  {filtered.length} / {counts.total} {t(C.count)}
                </>
              )}
            </span>
          </div>
                    ...
                    {pageRows.map((r) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="max-w-[120px] truncate px-2 py-2 text-[11px] text-slate-700">
                            {r.customer_label}
                          </td>
                          ...
                          <td className="px-2 py-2">
                            <span
                              className={cn(
                                ...
                                CONV_STATUS_PILL[r.conversation_status] ??
                                  "border-[#e8e6e0] bg-[#f5f4f0] text-slate-600",
                              )}
                            >
                          ...
                          <td className="px-2 py-2">
                            {hasEval ? (
                              <span className={cn("text-[12px] font-bold", scoreTextClass(r.overall_score!))}>
                                {r.overall_score!.toFixed(1)}
                              </span>
                            ) : (
                              <span className="text-[11px] text-slate-400">{t(C.detail.notEvaluated)}</span>
                            )}
                          </td>
                          <td className="px-2 py-2">
                            {hasEval ? (
                            ...
                            {hasEval ? (
                              <span
                                className={cn(
                                  ...
                                  STATUS_PILL[r.review_status!] ?? "",
                                )}
                              >
                                {r.needs_review ? t(C.pills.needsReview) : r.review_status}
                              </span>
                            ) : (
                              <span className="text-[11px] text-slate-400">{t(C.detail.notEvaluated)}</span>
                            )}
