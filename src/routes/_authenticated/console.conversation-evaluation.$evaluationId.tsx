...
const T = {
  en: {
    ...
    attempts: "Attempts",
    audit: "Audit",
    aiResponse: "Evaluated AI Reply",
    humanResponse: "Verified Human Response",
    noAi: "No AI reply found.",
    noHuman: "No identity-verified human response.",
    transcript: "Transcript",
    liveWarning: "Live transcript. Scores refer to the frozen snapshot.",
    qaFinding: "QA Finding",
    discrepancy: "Discrepancy Analysis",
    rootCause: "Root Cause Analysis",
    qaCase: "QA Case",
    createQaCase: "Create QA case",
    recordRootCause: "Record root cause",
    kbState: "KB publish / rollback",
    ...
    hashOk: "Snapshot verified",
    hashMismatch: "Snapshot hash mismatch — evidence may have drifted.",
    hashUnavailable: "No stored replay bundle for this attempt.",
    rawAdminOnly: "Raw evaluator payload is admin-only.",
    loading: "Loading…",
    noData: "—",
    provContract: "Contract",
    provModel: "Model",
    provPrompt: "Prompt",
    provDeployment: "Deployment",
    provKb: "KB evidence hash",
    provPolicy: "Policy evidence hash",
    provBundle: "Bundle hash",
    provSnapshot: "Snapshot hash",
    ...
    reviewReadOnly: "Read-only: requires admin or supervisor role.",
    ...
    refs: "Grounding",
    none: "None recorded.",
    noDetails: "No per-dimension detail rows.",
    ...
    aiClaim: "AI said",
    humanClaim: "Human said",
    groundedClaim: "Evidence supports",
    rootCauseEmpty: "No root cause recorded.",
    category: "Category",
    summary: "Summary",
    qaCaseEmpty: "No QA case linked.",
    caseTitle: "Case title",
    kbPublishEmpty: "No publish or rollback requested.",
    remoteSync: "Remote sync",
    remoteBlocked: "Remote sync to Nexus KB / SU CoachAI pending.",
    recorded: "Recorded",
    attemptsEmpty: "No attempts.",
    auditEmpty: "No audit entries.",
    retryEval: "Retry evaluation",
    status: "Status",
    error: "Error",
    started: "Started",
    deliveryEmpty: "Not queued for training.",
    deliveryRestricted: "Visible to admin/supervisor only.",
    deliveryAttempts: "Attempts",
    delivered: "Delivered",
    replayTranscript: "Transcript",
    replayCanonical: "Canonical Input",
    replayGrounding: "Grounding Evidence",
    replayRetention: "Retained until",
  ...
  zh: {
    ...
    audit: "稽核",
    aiResponse: "受評 AI 回覆",
    humanResponse: "已驗證人工回覆",
    noAi: "未找到 AI 回覆。",
    noHuman: "此對話沒有已驗證身分的人工回覆。",
    transcript: "對話內容",
    liveWarning: "即時對話內容，分數依據為凍結快照。",
    qaFinding: "QA 發現",
    discrepancy: "落差分析",
    rootCause: "根因分析",
    qaCase: "QA 案件",
    createQaCase: "建立 QA 案件",
    recordRootCause: "記錄根因",
    kbState: "知識庫發布/回滾",
    training: "訓練候選",
    kbGap: "知識庫缺口",
    improved: "改善後結果",
    ...
    reviewRecorded: "已記錄覆核",
    reviewReadOnly: "唯讀，需管理員或主管角色。",
    ...
    humanClaim: "人工回覆",
    groundedClaim: "證據支持",
    rootCauseEmpty: "沒有根因紀錄。",
    category: "類別",
    summary: "摘要",
    qaCaseEmpty: "尚未連結 QA 案件。",
    caseTitle: "案件標題",
    kbPublishEmpty: "尚未提出發布或回滾。",
    remoteSync: "遠端同步",
    remoteBlocked: "與 Nexus KB / SU CoachAI 的遠端同步待處理。",
    recorded: "已記錄",
    attemptsEmpty: "沒有執行紀錄。",
    auditEmpty: "沒有稽核紀錄。",
    retryEval: "重新評估",
    status: "狀態",
    error: "錯誤",
    started: "開始時間",
    deliveryEmpty: "尚未排入訓練佇列。",
    deliveryRestricted: "僅限管理員與主管檢視。",
    deliveryAttempts: "嘗試次數",
    delivered: "已派送",
    replayTranscript: "對話",
    replayCanonical: "標準輸入",
    replayGrounding: "依據證據",
    replayRetention: "保留至",
...
function Prov({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "2px 0" }}>
      ...
      <span
        style={{ fontFamily: "monospace", fontSize: 11, maxWidth: "60%", wordBreak: "break-all", textAlign: "right" }}
      >
        {value || "—"}
      </span>
    </div>
    ...
    <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {tab === "evaluation" && ev && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card title={t.score + " " + t.evaluation}>
            {details.length === 0 ? (
              <span style={{ color: "#9ca3af" }}>{t.noDetails}</span>
            ) : (
              <div>
                {details.map((dd: any) => (
                  <div
                    key={dd.evaluator_type}
                    style={{ border: "1px solid #f3f4f6", borderRadius: 6, padding: 10, marginBottom: 8 }}
                  >
                    ...
                    <div style={{ marginTop: 6, fontSize: 11, color: "#6b7280" }}>
                      <div>
                        <strong>{t.justification}:</strong> {dd.justification || t.none}
                      </div>
                      {dd.recommended_correction && (
                        <div style={{ marginTop: 4 }}>
                          <strong>{t.correction}:</strong> {dd.recommended_correction}
                        </div>
                      )}
          ...
          <Card title={t.reviewTitle}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              ...
              <Badge
                text={ev.review_status ?? "pending"}
                color={REVIEW_COLOR[ev.review_status] ?? "#6b7280"}
                bg="#f3f4f6"
              />
              {ev.reviewed_at && (
                <span style={{ fontSize: 11, color: "#9ca3af" }}>{new Date(ev.reviewed_at).toLocaleString()}</span>
              )}
            </div>
            {ev.review_note && (
              <div style={{ background: "#f9fafb", padding: 8, borderRadius: 6, fontSize: 11, marginBottom: 8 }}>
                {ev.review_note}
              </div>
            )}
            {!canReview ? (
              <div style={{ color: "#9ca3af" }}>{t.reviewReadOnly}</div>
            ) : (
              <>
                <textarea
                  value={reviewNote}
      ...
      {tab === "emotion" && (
        <Card title={t.emotion}>
          {(d?.emotion ?? []).length === 0 ? (
            <span style={{ color: "#9ca3af" }}>{t.emotionEmpty}</span>
          ) : (
            <div>
              {(d?.emotion ?? []).map((e: any) => {
      ...
      {tab === "next_steps" && (
        <Card title={t.next_steps}>
          {(d?.nextSteps ?? []).length === 0 ? (
            <span style={{ color: "#9ca3af" }}>{t.nextStepsEmpty}</span>
          ) : (
      ...
      {tab === "replay" && (
        <Card title={t.replay}>
          {!snapshot ? (
            <div style={{ color: "#9ca3af" }}>{t.hashUnavailable}</div>
          ) : (
            <>
                      ...
                      {(["transcript", "canonical", "grounding"] as const).map((s) => (
                        <button
                          ...
                          style={{
                            ...
                            background: sub === s ? "#eff6ff" : "#fff",
                            color: sub === s ? "#2563eb" : "#374151",
                          }}
                        >
                          {
                            t[
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card title={t.discrepancy}>
            {(d?.discrepancies ?? []).length === 0 ? (
              <span style={{ color: "#9ca3af" }}>{t.discrepancyEmpty}</span>
            ) : (
              (d?.discrepancies ?? []).map((disc: any) => (
                <div
                  key={disc.id}
                  style={{ border: "1px solid #f3f4f6", borderRadius: 6, padding: 8, marginBottom: 6 }}
                >
          ...
          <Card title={t.rootCause}>
            {(d?.rootCauses ?? []).length === 0 ? (
              <span style={{ color: "#9ca3af" }}>{t.rootCauseEmpty}</span>
            ) : (
                ...
                <div key={r.id} style={{ border: "1px solid #f3f4f6", borderRadius: 6, padding: 8, marginBottom: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <span style={{ fontSize: 11, color: "#9ca3af" }}>
                      {t.remoteSync}: {r.remote_sync_state}
                    </span>
                  </div>
          ...
          <Card title={t.qaCase}>
            {(d?.qaCases ?? []).length === 0 ? (
              <span style={{ color: "#9ca3af" }}>{t.qaCaseEmpty}</span>
            ) : (
              (d?.qaCases ?? []).map((q: any) => (
                <div
                  key={q.id}
                ...
                >
                  ...
                  <span style={{ marginLeft: "auto", fontSize: 11, color: "#9ca3af" }}>
                    {t.remoteSync}: {q.remote_sync_state}
                  </span>
                </div>
              ))
            )}
      ...
      {tab === "integration" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card title={t.training}>
            {(d?.trainingLinks ?? []).length === 0 ? (
              <span style={{ color: "#9ca3af" }}>{t.deliveryEmpty}</span>
            ) : (
              (d?.trainingLinks ?? []).map((l: any) => (
                <div key={l.id} style={{ border: "1px solid #f3f4f6", borderRadius: 6, padding: 8, marginBottom: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontWeight: 600 }}>
                      {l.link_kind === "training_candidate" ? t.training : t.kbGap}
                    </span>
                    <span style={{ fontSize: 11, color: "#9ca3af" }}>
                      {t.remoteSync}: {l.remote_sync_state}
                    </span>
                  </div>
          ...
          <Card title={t.kbState}>
            {(d?.kbPublish ?? []).length === 0 ? (
              <span style={{ color: "#9ca3af" }}>{t.kbPublishEmpty}</span>
            ) : (
              (d?.kbPublish ?? []).map((k: any) => (
                ...
                >
                  ...
                  <span>{k.state}</span>
                  <span style={{ marginLeft: "auto", fontSize: 11, color: "#9ca3af" }}>
                    {t.remoteSync}: {k.remote_sync_state}
                  </span>
          ...
          <Card title={t.delivered}>
            {!canReview ? (
              <span style={{ color: "#9ca3af" }}>{t.deliveryRestricted}</span>
            ) : (d?.outbox ?? []).length === 0 ? (
              <span style={{ color: "#9ca3af" }}>{t.deliveryEmpty}</span>
            ) : (
              (d?.outbox ?? []).map((o: any) => (
                <div key={o.id} style={{ display: "flex", gap: 8, alignItems: "center", padding: "4px 0" }}>
                  ...
                  <span>{o.status}</span>
                  <span>
                    {t.deliveryAttempts}: {o.delivery_attempts}/{o.max_attempts}
                  </span>
                  {o.delivered_at && (
                    <span>
                      {t.delivered}: {new Date(o.delivered_at).toLocaleString()}
                    </span>
                  )}
                </div>
              ))
      ...
      {tab === "attempts" && (
        <Card title={t.attempts}>
          {(d?.attempts ?? []).length === 0 ? (
            <span style={{ color: "#9ca3af" }}>{t.attemptsEmpty}</span>
          ) : (
            (d?.attempts ?? []).map((a: any) => (
              ...
              >
                ...
                <span>
                  {t.started}: {new Date(a.created_at).toLocaleString()}
                </span>
                {a.error_message && (
                  <span style={{ color: "#dc2626" }}>
                    {t.error}: {a.error_message}
                  </span>
                )}
              </div>
            ))
      ...
      {tab === "audit" && (
        <Card title={t.audit}>
          {(d?.audit ?? []).length === 0 ? (
            <span style={{ color: "#9ca3af" }}>{t.auditEmpty}</span>
          ) : (
            (d?.audit ?? []).map((a: any) => (
