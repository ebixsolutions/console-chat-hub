...
function ConsoleFeedbackAutomation() {
  ...
  const [templates, setTemplates] = useState<Record<RatingType, MessageTemplate>>(DEFAULT_TEMPLATES);
  ...
  const handleSave = async () => {
    ...
    // C3/C4: Override updated_by with real user identity before save.
    // Preferred: "displayName <email>" → email → displayName → "System"
    // demoRole is UI-only and must never be used for audit identity.
    let realUpdatedBy = "System";
    try {
      ...
      if (authData?.user) {
        ...
        if (dn && authEmail) {
          realUpdatedBy = `${dn} <${authEmail}>`;
        } else if (authEmail) {
          realUpdatedBy = authEmail;
        } else if (dn) {
          realUpdatedBy = dn;
        }
      }
  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>Post-Resolution Feedback Automation</div>
        <div style={{ fontSize: 11, color: "#888", marginBottom: 12 }}>對話結案後自動發送客戶評分請求</div>
        <label
          style={{
            display: "flex",
            alignItems: "center",
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {RATING_TYPES.map((r) => (
            ...
            >
              ...
              />
              {r.label}
              {r.suffix && r.suffixStyle === "plain" && <span style={{ fontSize: 9, color: "#888" }}>{r.suffix}</span>}
      ...
      <div style={cardStyle}>
          ...
          {TEMPLATE_TABS.map((tb) => (
            <button
              ...
              style={{
                ...
                border: "none",
                cursor: "pointer",
                borderBottom: templateTab === tb.key ? "2px solid #1a1a1a" : "2px solid transparent",
                marginBottom: -1,
                whiteSpace: "nowrap",
              }}
            >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div>
            ...
            <input value={tpl.subject} onChange={(e) => updateTpl("subject", e.target.value)} style={inputStyle} />
            ...
            <input value={tpl.ctaEn} onChange={(e) => updateTpl("ctaEn", e.target.value)} style={inputStyle} />
            ...
            {templateTab === "survey" && (
              <div style={{ marginTop: 14 }}>
                ...
                {surveyQuestions.map((q, i) => (
                  ...
                  >
                    <span style={{ fontSize: 10, color: "#888", width: 20 }}>Q{i + 1}</span>
                    {q.fixed ? (
                      <div
                        style={{
                          flex: 1,
                      ...
                      >
                        {q.text} <span style={{ fontSize: 9.5, color: "#888" }}>({q.type}) — fixed</span>
                      </div>
                    ) : (
                      <>
                        <input
                          ...
                          onChange={(e) =>
                            setSurveyQuestions((prev) =>
                              prev.map((sq, si) => (si === i ? { ...sq, text: e.target.value } : sq)),
                            )
                          }
            ...
            {previewMode === "email" ? (
              ...
              >
                ...
                <div style={{ padding: "12px 14px" }}>
                  <div style={{ fontSize: 12, color: "#1a1a1a", marginBottom: 10 }}>Hi Frankie,</div>
                  <div style={{ fontSize: 12, color: "#555", lineHeight: 1.6, marginBottom: 14 }}>{tpl.bodyEn}</div>
                  <div
                    style={{
