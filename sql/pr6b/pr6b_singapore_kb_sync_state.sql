-- PR-6B Singapore KB sync local state coordination.
...
BEGIN
  ...
  WHERE l.link_kind='training_candidate'
    ...
    AND NOT EXISTS (
      ...
      WHERE s.evaluation_id=l.evaluation_id AND s.action='publish'
        AND s.state IN ('in_progress','published','failed'))
  ORDER BY l.created_at
  FOR UPDATE SKIP LOCKED LIMIT 1;
  IF NOT FOUND THEN RETURN jsonb_build_object('result','none'); END IF;

  INSERT INTO public.ce_kb_publish_state(evaluation_id,company_id,kb_document_ref,action,state,requested_by,remote_sync_state,updated_at)
  VALUES(v.evaluation_id,v.company_id,COALESCE(v.improved_result->'kb_update'->>'document_id',''),'publish','in_progress',
         '00000000-0000-0000-0000-000000000000'::uuid,'pending',now())
  ON CONFLICT (evaluation_id,action) DO UPDATE SET state='in_progress',updated_at=now(),last_error=NULL
  RETURNING id INTO v_state_id;

  RETURN jsonb_build_object('result','claimed','training_link_id',v.id,'evaluation_id',v.evaluation_id,
    'company_id',v.company_id,'improved_result',v.improved_result,'payload',v.payload,'publish_state_id',v_state_id);
END $f$;

CREATE OR REPLACE FUNCTION public.finish_pr6b_kb_sync_tx(p_training_link_id uuid,p_success boolean,p_remote_ref text,p_error text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public','pg_temp' AS $f$
DECLARE v record;
BEGIN
  SELECT id,evaluation_id,company_id INTO v FROM public.ce_training_link WHERE id=p_training_link_id FOR UPDATE;
  IF NOT FOUND THEN RETURN jsonb_build_object('result','not_found'); END IF;
  UPDATE public.ce_kb_publish_state
    SET state=CASE WHEN p_success THEN 'in_progress' ELSE 'failed' END,
        remote_sync_state=CASE WHEN p_success THEN 'pending' ELSE 'failed' END,
        remote_ref=COALESCE(p_remote_ref,remote_ref),last_error=left(p_error,500),updated_at=now()
  WHERE evaluation_id=v.evaluation_id AND action='publish';
