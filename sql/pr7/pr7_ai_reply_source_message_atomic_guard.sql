-- PR-7 AI reply source-message atomic freshness guard.
-- SOURCE ONLY. Do not apply to production without explicit Director authorization.
-- Requires the existing PR-3 commit_ai_reply_tx contract.
...
BEGIN
  ...
  -- receive_widget_message_tx takes the same conversations row FOR UPDATE.
  -- While this lock is held, no newer visitor message transaction can commit
  -- ahead of this check. Reject a reply for A if visitor message B already
  -- exists after A, including deterministic UUID ordering for equal timestamps.
  IF EXISTS (
    SELECT 1
...
GRANT EXECUTE ON FUNCTION public.commit_ai_reply_tx(uuid,uuid,text,jsonb)
  TO service_role;
