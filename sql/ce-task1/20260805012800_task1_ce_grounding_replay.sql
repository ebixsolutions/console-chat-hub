...
DECLARE v_n integer;
BEGIN
  DELETE FROM public.ce_replay_bundle WHERE retention_expires_at < now();
...
-- ---------------------------------------------------------------------------
-- 6. Canonical CE status semantics — single source of truth for the UI
--      needs_review   : canonical review-required state
--      training_ready : accepted + training eligible + not delivered
...
SELECT
  ...
  o.delivered_at,
  (e.severity IN ('critical','high') OR e.overall_score < 70)          AS needs_review,
  (e.training_eligible AND COALESCE(o.status,'none') <> 'delivered')   AS training_ready,
...
-- ---------------------------------------------------------------------------
-- 7. Feature flags for this slice — default OFF, no behaviour change on apply
-- ---------------------------------------------------------------------------
