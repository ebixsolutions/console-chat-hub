...
function identity(candidate, expected) {
  if (
    !candidate || !GIT40.test(candidate.head) || !GIT40.test(candidate.tree) ||
    candidate.head !== expected.head || candidate.tree !== expected.tree
  ) fail("deterministic_cross_release");
  if (
    !present(candidate.run_id) || !Number.isInteger(candidate.run_attempt) ||
...
export function verifyObjectiveOracle({ dataset, overlay, freeze }) {
  ...
  if (
    ...
    overlay.responses_at_freeze !== 0 || freeze.responses_at_freeze !== 0
  ) fail("objective_response_before_freeze");
  if (
    overlay.quality_score !== "NOT_MEASURED" ||
    overlay.quality_denominator_eligible !== false ||
    freeze.quality_score !== "NOT_MEASURED" || freeze.product_ready !== false
  ) fail("objective_quality_gate_weakened");
  if (
    !Array.isArray(dataset.cases) || dataset.cases.length !== 100 ||
    !Array.isArray(overlay.cases) || overlay.cases.length !== 100
  ) fail("objective_case_count_invalid");
  const source = new Map(dataset.cases.map((row) => [row.case_id, row]));
  const seen = new Set(), endpoints = new Map();
  for (const row of overlay.cases) {
    if (
      !source.has(row.case_id) || seen.has(row.case_id) ||
    ...
    if (
      !Array.isArray(row.p0_prohibitions) || row.p0_prohibitions.length < 9 ||
  ...
  if (
    endpoints.get("generate-reply") !== 50 ||
    endpoints.get("agent-assist") !== 50
...
export function verifyObjectiveRuntime(
  { runtime, dataset, overlay, freeze, expectedCandidate },
) {
  ...
  if (
    !runtime.engine || !HEX64.test(runtime.engine.rule_pack_sha256) ||
  ...
  if (
    "weighted_score" in runtime || "quality_score" in runtime ||
    "pass" in runtime
  ) fail("objective_quality_aggregate_forbidden");
  if (
    !Array.isArray(runtime.observations) || runtime.observations.length !== 100
  ) fail("objective_runtime_count_invalid");
  const oracles = new Map(overlay.cases.map((row) => [row.case_id, row]));
  const seen = new Set(), responseHashes = new Set();
  let conforming = 0;
  for (const row of runtime.observations) {
    ...
    if (
      row.p0_labels.length === 0 && row.forbidden_transition_count === 0 &&
...
export function generateFullBlindPacket(
  { dataset, runtime, rubric, packetId = crypto.randomUUID() },
) {
  if (
    !Array.isArray(runtime.observations) || runtime.observations.length !== 100
  ...
  const samples = runtime.observations.map((row) => {
    if (
      !cases.has(row.case_id) || seen.has(row.case_id) ||
...
export function verifyFullHumanReview(
  { artifact, packet, expectedCandidate, rubric },
) {
  if (
  ...
  if (
    ...
    !Array.isArray(packet.samples) || packet.samples.length !== 100
  ...
  for (const reviewer of artifact.reviewers) {
    if (
      !present(reviewer.reviewer_id) || reviewers.has(reviewer.reviewer_id) ||
      reviewer.human_attestation !== true || !present(reviewer.provenance) ||
    ...
    if (
      ...
      [...forbiddenRoles].some((role) =>
        !reviewer.excluded_roles_attestation.includes(role)
      )
  ...
  const grouped = new Map(), reviewIds = new Set();
  for (const review of artifact.reviews) {
    const sample = samples.get(review.sample_id);
    if (
      !sample || !reviewers.has(review.reviewer_id) ||
    ...
    if (
      review.human_submitted !== true || review.blind_attestation !== true ||
      !/^\d{4}-\d{2}-\d{2}T/.test(review.timestamp) ||
      !present(review.reason) || review.reason.trim().split(/\s+/).length < 4
    ) fail(`full_human_review_provenance_invalid:${review.review_id}`);
    if (
      !review.scores ||
      Object.keys(review.scores).sort().join("|") !==
        [...DIMENSIONS].sort().join("|")
    ) fail(`full_human_dimensions_invalid:${review.review_id}`);
    for (const dimension of DIMENSIONS) {
      if (
        !Number.isFinite(review.scores[dimension]) ||
        review.scores[dimension] < 0 || review.scores[dimension] > 10
      ) fail(`full_human_score_invalid:${review.review_id}:${dimension}`);
    }
    if (
      !Array.isArray(review.p0_labels) || review.p0_labels.some((label) =>
        !rubric.critical_p0.includes(label)
  ...
  for (const sample of packet.samples) {
    ...
    const conflict = DIMENSIONS.some((dimension) =>
      Math.abs(first.scores[dimension] - second.scores[dimension]) > 2
    ) || first.recommendation !== second.recommendation ||
      [...new Set([...first.p0_labels, ...second.p0_labels])].some((label) =>
        first.p0_labels.includes(label) !== second.p0_labels.includes(label)
      );
    if (
      conflict &&
      (rows.length < 3 || rows[2].adjudicator !== true ||
  ...
  const dimensionScores = Object.fromEntries(
    DIMENSIONS.map((
      dimension,
    ) => [dimension, Number((sums[dimension] / 100).toFixed(6))]),
  );
  const weightedScore = Number(
    (DIMENSIONS.reduce(
      (total, dimension) =>
        total + dimensionScores[dimension] * rubric.weights[dimension],
      0,
    ) / 10)
      .toFixed(6),
  );
  const pass = weightedScore >= rubric.target.weighted_score_min && p0 === 0 &&
    DIMENSIONS.every((dimension) =>
      dimensionScores[dimension] >=
        rubric.target.dimension_average_min[dimension]
    );
  return {
...
if (command === "verify-oracle") {
  ...
  const [artifactPath, packetPath, rubricPath, expectedHead, expectedTree] =
    args;
