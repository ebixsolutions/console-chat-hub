...
import { canonicalJson, loadFrozenDataset, validateIsolation, verifyEvidence } from "./c3_nonproduction_external_quality.mjs";
...
assert.throws(()=>validateIsolation({projectRef:"nrfxhqabwblzxoushgnm",projectUrl:"https://nrfxhqabwblzxoushgnm.supabase.co",head:dataset.value.candidate.head,tree:dataset.value.candidate.tree}),/nonproduction_project_ref_invalid/);
assert.throws(()=>validateIsolation({projectRef:"nbtowfuvvfqpxqydyoby",projectUrl:"https://nrfxhqabwblzxoushgnm.supabase.co",head:dataset.value.candidate.head,tree:dataset.value.candidate.tree}),/nonproduction_url_identity_invalid/);
...
const evidence={schema_version:"c3-nonproduction-http-evidence-1.0.0",binding:{projectRef:"nbtowfuvvfqpxqydyoby",projectUrl:"https://nbtowfuvvfqpxqydyoby.supabase.co",head:dataset.value.candidate.head,tree:dataset.value.candidate.tree,dataset_sha256:dataset.sha256},observations,cleanup:{checked_ids:100,zero_residual:true}};
