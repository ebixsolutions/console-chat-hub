...
for f in "$MIG" "$SUBMIT" "$DELIVER"; do test -s "$f" || { echo "HF2_FAIL=missing_or_empty:$f"; exit 1; }; done
...
# Do not use pg_isready alone: postgres can accept connections before POSTGRES_DB
# initialization has finished. Gate on an actual query against hf2.
READY=0
...
BEGIN
 ...
 r:=set_conversation_resolution_tx(cv,co,usr,ag,'resolved','again'); SELECT count(*) INTO cnt FROM feedback_request WHERE conversation_id=cv; IF r->>'result'<>'already_in_state' OR cnt<>1 THEN RAISE EXCEPTION 'resolution_idempotency:%/%',r,cnt; END IF;
