...
[ -n "${SUPABASE_ACCESS_TOKEN:-}" ] || stop "SUPABASE_ACCESS_TOKEN missing"
cd "$REPO"
...
for f in   sql/pr6b/pr6b_training_result_ingest.sql   sql/pr6b/pr6b_singapore_kb_sync_state.sql   sql/pr6b/pr6b_singapore_kb_finalize.sql   supabase/functions/training-outbox-worker/index.ts   supabase/functions/training-result-receiver/index.ts   supabase/functions/training-kb-sync/index.ts   supabase/functions/training-kb-finalize/index.ts   supabase/functions/_shared/kb-auth.ts   supabase/functions/_shared/kb-client.ts   supabase/functions/_shared/kb-aggregation-response.ts
do
...
SECRET_LIST="$(npx supabase secrets list --project-ref "$PROJECT_REF" 2>/dev/null)" || stop "unable to read Edge secret inventory"
required=(TRAINING_OUTBOX_INTERNAL_TOKEN SU_COACHAI_EVALUATION_ENDPOINT SU_COACHAI_AUTH_HEADER SU_COACHAI_AUTH_VALUE SU_COACHAI_RESULT_TOKEN TRAINING_KB_SYNC_INTERNAL_TOKEN KB_SINGAPORE_TENANT_MAP_JSON KB_SINGAPORE_TENANT_API_KEYS_JSON)
for n in "${required[@]}"; do
  printf '%s\n' "$SECRET_LIST" | awk '{print $1}' | grep -Fxq "$n" || stop "production Edge secret name missing: $n"
done
...
for fn in training-outbox-worker training-result-receiver training-kb-sync training-kb-finalize; do
  npx supabase functions deploy "$fn" --project-ref "$PROJECT_REF" || stop "Edge deployment failed: $fn"
done

echo "W2 TASK 2.3 LEARNING LOOP ACTIVATION: DEPLOYED"
