...
def must_sub(s: str, pattern: str, replacement: str, label: str, flags=0) -> str:
    out, n = re.subn(pattern, lambda m: replacement, s, count=1, flags=flags)
    if n != 1:
        raise SystemExit(f"MISSING_PATTERN:{label}:{n}")
    return out
...
s = must_sub(
    s,
    r'const EXPLICIT_ZH = /.*?/;\nconst EXPLICIT_EN = /.*?/i;',
    r'''const EXPLICIT_ZH = /(?:而家|現在|现在|即刻|立即).{0,8}(轉|转|接|搵|找|聯絡|联系).{0,8}(真人|人工|客服(?:人員|人员)?)|(?:請|请|麻煩|麻烦|幫我|帮我).{0,10}(轉|转|接|搵|找|聯絡|联系).{0,8}(真人|人工|客服(?:人員|人员)?)|(?:我要|我想|我需要|想要|需要).{0,8}(真人|人工|客服(?:人員|人员)?)/;
...
'''
if marker not in s: raise SystemExit("MISSING_PATTERN:memory_inject")
...
s = p.read_text()
s = must_sub(s,
    r'function extractExplicitJurisdictionConstraint\(text: string\): string \| null \{.*?\n\}\n\nfunction evidenceSupportsJurisdiction',
    r'''function extractExplicitJurisdictionConstraint(text: string): string | null {
...
# 5) CE bounded retry + deterministic emotion fallback.
p = Path("supabase/functions/_shared/ce-automation-engine.ts")
s = p.read_text()
s = must_sub(s,
    r'async function runEvaluator\(.*?\n\}\n\nasync function runSignals\(',
