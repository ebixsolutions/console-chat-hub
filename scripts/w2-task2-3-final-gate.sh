...
# Source contracts — all Product-ready learning-loop guards are mandatory.
...
for f in \
  ...
  tests/edge/product-ready-learning-approval-guard-contract.py \
  tests/edge/product-ready-learning-rag-readback-integrity-contract.py
do
  test -s "$f" || { echo "FAIL missing/empty required Task2.3 file: $f" >&2; exit 1; }
done
